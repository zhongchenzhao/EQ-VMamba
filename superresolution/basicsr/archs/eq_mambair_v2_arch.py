...
# -*- coding: utf-8 -*-
"""
@File  : eq_mambair_v2_arch.py
@Author: ZhongchenZhao
@Date  : 2025/11/9 09:10
@Desc  : 
"""
    ...
    def __init__(self, num_feat, is_light_sr=True, compress_ratio=3, tranNum=4, squeeze_factor=60.0/(2*4)):
        ...
        if is_light_sr: # we use dilated-conv & DWConv for lightSR for a large ERF, True
            ...
            self.cab = nn.Sequential(
                ...
                PermuteLayer(0, 3, 1, 2),
                GroupFconv_PCA(sizeP=3, inNum=num_feat // (tranNum*compress_ratio), outNum=num_feat // (tranNum*compress_ratio),
                ...
                PermuteLayer(0, 3, 1, 2),
                GroupFconv_PCA(3, inNum=num_feat // tranNum, outNum=num_feat // tranNum,
            ...
            self.cab = nn.Sequential(
                ...
                Fconv_PCA(3, inNum=num_feat//tranNum, outNum=num_feat // (tranNum*compress_ratio),
                          tranNum=tranNum, padding=1, bias=True),
                nn.GELU(),
                # nn.Conv2d(num_feat // compress_ratio, num_feat, 3, 1, 1),
                Fconv_PCA(3, inNum=num_feat//(tranNum*compress_ratio), outNum=num_feat // tranNum,
                          tranNum=tranNum, padding=1, bias=True),
                EQChannelAttention(num_feat, squeeze_factor)
            )
...
class EQSS2D(nn.Module):
    def __init__(
        ...
        self.eq_out_proj = EQ_linear_inter(inNum=self.d_inner // tranNum, outNum=self.d_model // tranNum,
                                           tranNum=tranNum, bias=bias)  # bias=False
        self.dropout = EQDropout(dropout) if dropout > 0. else None  # dropout=0
...
class EQResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':      # True
            ...
            self.eqconv = Fconv_PCA(5, inNum=dim//tranNum, outNum=dim //tranNum, tranNum=tranNum, padding=2, bias=True)
            ...
            self.eqconv = nn.Sequential(
                Fconv_PCA(5, inNum=dim//tranNum, outNum=dim // (tranNum*4), tranNum=tranNum, padding=2, bias=True),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PermuteLayer(0, 2, 3, 1),
                EQ_linear_inter(dim // (tranNum*4), dim // (tranNum*4), tranNum=tranNum, bias=True),
                PermuteLayer(0, 3, 1, 2),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Fconv_PCA(5, inNum=dim//(tranNum*4), outNum=dim//tranNum, tranNum=tranNum, padding=2, bias=True),
            )
    ...
    def __init__(self, scale, num_feat, tranNum=4):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                m.append(Fconv_PCA(3, inNum=num_feat//tranNum, outNum=4*num_feat//tranNum, tranNum=tranNum, padding=1, bias=True))
                m.append(EQPixelShuffle(scale=2, tranNum=tranNum))
        elif scale == 3:
            m.append(Fconv_PCA(3, inNum=num_feat//tranNum, outNum=9 * num_feat//tranNum, tranNum=tranNum, padding=1, bias=True))
...
class EQUpsampleOneStepv2(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, tranNum=4):
        ...
        m.append(Fconv_PCA(5, inNum=num_feat // tranNum, outNum=(scale**2) * num_out_ch, tranNum=tranNum, padding=2, bias=True))
        m.append(EQPixelShuffle(scale, tranNum=tranNum))
        m.append(TranAvgPool(tranNum=tranNum))
        super(EQUpsampleOneStepv2, self).__init__(*m)
...
class EQMambaIRv2(nn.Module):
    ...
    def __init__(self,
                 ...
                 resi_connection='1conv',
                 **kwargs):
        super(EQMambaIRv2, self).__init__()
        tranNum = 4
        num_in_ch = in_chans
        num_out_ch = in_chans
        num_feat = 64
        ...
        self.eqconv_firstv2 = Fconv_PCA(5, inNum=num_in_ch, outNum=embed_dim // tranNum,
                                      tranNum=tranNum, padding=2, ifIni=1,  bias=True)
        ...
        self.pos_drop = EQDropout(p=drop_rate)
        ...
        if resi_connection == '1conv':
            ...
            self.eqconv_after_bodyv2 = Fconv_PCA(5, inNum=embed_dim // tranNum, outNum=embed_dim // tranNum,
                                               tranNum=tranNum, padding=2, bias=True)
            ...
            # )
            self.eqconv_after_bodyv2 = nn.Sequential(
                Fconv_PCA(5, inNum=embed_dim // tranNum, outNum=embed_dim // (tranNum * 4), tranNum=tranNum, padding=2, bias=True),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PermuteLayer(0, 2, 3, 1),
                EQ_linear_inter(embed_dim // (tranNum * 4), embed_dim // (tranNum * 4), tranNum=tranNum, bias=True),
                PermuteLayer(0, 3, 1, 2),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Fconv_PCA(5, inNum=embed_dim // (tranNum * 4), outNum=embed_dim // tranNum, tranNum=tranNum, padding=2, bias=True),
            )
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            self.eqconv_before_upsample = nn.Sequential(
                Fconv_PCA(3, inNum=embed_dim // tranNum, outNum=num_feat // tranNum, tranNum=tranNum, padding=1, bias=True),
                nn.LeakyReLU(inplace=True))

            self.equpsample = EQUpsample(upscale, num_feat)
            # self.conv_last = nn.Conv2d(num_feat, num_out_ch, 3, 1, 1)
            self.eqconv_last = Fconv_PCA_out(3, inNum=num_feat // tranNum, outNum=num_out_ch, tranNum=tranNum, padding=1, bias=True),
        elif self.upsampler == 'pixelshuffledirect':            # True
            # for lightweight SR (to save parameters)
            self.equpsamplev2 = EQUpsampleOneStepv2(upscale, embed_dim, num_out_ch, tranNum=tranNum)

        else:
            # for image denoising
            # self.conv_last = nn.Conv2d(embed_dim, num_out_ch, 3, 1, 1)
            self.eqconv_last = Fconv_PCA_out(3, inNum=embed_dim // tranNum, outNum=num_out_ch,
                                               tranNum=tranNum, padding=1, bias=True)
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            # for classical SR
            x = self.eqconv_firstv2(x)
            x = self.eqconv_after_bodyv2(self.forward_features(x)) + x
            ...
            x = self.eqconv_firstv2(x)
            x = self.eqconv_after_bodyv2(self.forward_features(x)) + x
            x = self.equpsamplev2(x)

        else:
            # for image denoising
            x_first = self.eqconv_firstv2(x)
            res = self.eqconv_after_bodyv2(self.forward_features(x_first)) + x_first
...
if __name__ == "__main__":
    ...
    load_path = r"/mnt/zzc/projects/MambaIR/experiments/EQMambaIR_lightSR_x2/models/net_g_500000.pth"
    load_net = torch.load(load_path, map_location=lambda storage, loc: storage)
    print("load_net ", load_net.keys())
    ...
    x = torch.randn([1, 3, 188, 205], device=device)
    net_g.eval()
    y = net_g(x)
