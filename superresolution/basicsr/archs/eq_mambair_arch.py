...
# -*- coding: utf-8 -*-
"""
@File  : eq_mambair_arch.py
@Author: ZhongchenZhao
@Date  : 2025/10/31 08:31
@Desc  : 
"""
...
class EQResidualGroup(nn.Module):
    ...
    def __init__(self,
        ...
        if resi_connection == '1conv':      # True
            ...
            self.eqconv = Fconv_PCA(3, inNum=dim//tranNum, outNum=dim //tranNum, tranNum=tranNum, padding=1, bias=True)
            ...
            self.eqconv = nn.Sequential(
                Fconv_PCA(3, inNum=dim//tranNum, outNum=dim // (tranNum*4), tranNum=tranNum, padding=1, bias=True),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PermuteLayer(0, 2, 3, 1),
                EQ_linear_inter(dim // (tranNum*4), dim // (tranNum*4), tranNum=tranNum, bias=True),
                PermuteLayer(0, 3, 1, 2),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Fconv_PCA(3, inNum=dim//(tranNum*4), outNum=dim//tranNum, tranNum=tranNum, padding=1, bias=True),
            )
...
class EQUpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, tranNum=4):
        ...
        m.append(Fconv_PCA(3, inNum=num_feat // tranNum, outNum=(scale**2) * num_out_ch, tranNum=tranNum, padding=1, bias=True))
...
class EQMambaIR(nn.Module):
    ...
    def __init__(self,
        ...
        self.eqconv_first = Fconv_PCA(3, inNum=num_in_ch, outNum=embed_dim // tranNum,
                                               tranNum=tranNum, padding=1, ifIni=1,  bias=True)
        ...
        if resi_connection == '1conv':
            ...
            self.eqconv_after_body = Fconv_PCA(3, inNum=embed_dim // tranNum, outNum=embed_dim // tranNum,
                                               tranNum=tranNum, padding=1, bias=True)
            ...
            self.eqconv_after_body = nn.Sequential(
                Fconv_PCA(3, inNum=embed_dim // tranNum, outNum=embed_dim // (tranNum * 4), tranNum=tranNum, padding=1, bias=True),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                PermuteLayer(0, 2, 3, 1),
                EQ_linear_inter(embed_dim // (tranNum * 4), embed_dim // (tranNum * 4), tranNum=tranNum, bias=True),
                PermuteLayer(0, 3, 1, 2),
                nn.LeakyReLU(negative_slope=0.2, inplace=True),
                Fconv_PCA(3, inNum=embed_dim // (tranNum * 4), outNum=embed_dim // tranNum, tranNum=tranNum, padding=1, bias=True),
            )

        # -------------------------3. high-quality image reconstruction ------------------------ #
        if self.upsampler == 'pixelshuffle':            # classical
            # for classical SR
        ...
        elif self.upsampler == 'pixelshuffledirect':            # lightweight: True
