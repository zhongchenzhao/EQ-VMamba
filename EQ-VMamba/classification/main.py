...
def parse_option():
    ...
    parser.add_argument('--EQ_tranNum', type=int, default=4, help="EQ_tranNum")
    parser.add_argument('--num_classes', type=int, default=1000, help="num_classes")
    parser.add_argument('--test_transform_angle', type=int, default=0, help="test_transform_angle")
...
def main(config, args):
    ...
    if config.MODEL.RESUME:
        ...
        if model_ema is not None:
            ...
            logger.info(f"Accuracy of the network ema on the {len(dataset_val)} test images: {acc1_ema:.3f}%")
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        acc1, acc5, loss = validate(config, data_loader_val, model)
        logger.info(f"Accuracy of the network on the {len(dataset_val)} test images (epoch: {epoch}): {acc1:.3f}%")
        max_accuracy = max(max_accuracy, acc1)
        logger.info(f'==> Max accuracy: {max_accuracy:.3f}%, best epoch: {max_accuracy_epoch}')
        if acc1 == max_accuracy:
            max_accuracy_epoch = epoch
            save_checkpoint_ema(config, "best", model_without_ddp, max_accuracy, optimizer,
                                lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema)

        if model_ema is not None:
            acc1_ema, acc5_ema, loss_ema = validate(config, data_loader_val, model_ema.ema)
            logger.info(f"Accuracy of the network on the {len(dataset_val)} test images: {acc1_ema:.3f}%")
            max_accuracy_ema = max(max_accuracy_ema, acc1_ema)
            if acc1_ema == max_accuracy_ema:
                max_accuracy_ema_epoch = epoch
                save_checkpoint_ema(config, "ema_best", model_without_ddp, max_accuracy, optimizer,
                                    lr_scheduler, loss_scaler, logger, model_ema, max_accuracy_ema)
            logger.info(f'==> Max accuracy ema: {max_accuracy_ema:.3f}, best epoch: {max_accuracy_ema_epoch}')
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler,
                    model_ema=None, model_time_warmup=50):
    model.train()
    optimizer.zero_grad()
