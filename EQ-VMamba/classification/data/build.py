...
def build_dataset(is_train, config):
    ...
    elif config.DATA.DATASET == 'imagenet100rotate':
        prefix = 'train' if is_train else 'val_rotate'
        if config.DATA.ZIP_MODE:
            ann_file = prefix + "_map.txt"
            prefix = prefix + ".zip@/"
            dataset = CachedImageFolder(config.DATA.DATA_PATH, ann_file, prefix, transform,
...
def build_transform(is_train, config):
    ...
    RandAugment(n=2, ops=               # RandAugment 自动在多个图像操作中随机选取 n=2 个增强操作应用到每张图像。
    ...
    if config.test_transform_angle:
        t.append(TransformsRotate(angle=config.test_transform_angle))
...
class TransformsRotate:
    def __init__(self, angle=0):
        super(TransformsRotate, self).__init__()
        self.angle = angle
