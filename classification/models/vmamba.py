...
# 20251005: from mamba_init, same ABCD for 4 scanning routs
class mamba_initv4:
    @staticmethod
    def dt_init(dt_rank, d_inner, dt_scale=1.0, dt_init="random", dt_min=0.001, dt_max=0.1, dt_init_floor=1e-4):
        dt_proj = nn.Linear(dt_rank, d_inner, bias=True)
    ...
    def init_dt_A_D(cls, d_state, dt_rank, d_inner, dt_scale, dt_init, dt_min, dt_max, dt_init_floor, k_group=4):
        ...
        dt_projs_weight = nn.Parameter(torch.stack([t.weight for t in dt_projs], dim=0))  # (K, inner, rank)
        dt_projs_weight = dt_projs_weight.repeat(k_group, 1, 1)
        dt_projs_bias = nn.Parameter(torch.stack([t.bias for t in dt_projs], dim=0))  # (K, inner)
        dt_projs_bias = dt_projs_bias.repeat(k_group, 1, 1)
        del dt_projs
...
# postfix: _onsigmoid,_onsoftmax,_ondwconv3,_onnone;_nozact,_noz;_oact;_no32;
# history support: v2,v3;v31d,v32d,v32dc;
# 20251005: from SS2Dv2, same ABCD for 4 scanning routs
class SS2Dv4:
    def __initv4__(
            self,
            # basic dims ===========
        ...
        # forward_type debug =======================================
        self.forward_core = self.forward_corev4              # 20251005
        ...
        # x proj ============================
        self.x_proj = [nn.Linear(self.d_inner, (self.dt_rank + self.d_state * 2), bias=False)]      # 20251005
        self.x_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_proj], dim=0)) # (K, N, inner)
        ...
        if initialize in ["v0"]:
            self.A_logs, self.Ds, self.dt_projs_weight, self.dt_projs_bias = mamba_initv4.init_dt_A_D(
    ...
    def forward_corev4(
        ...
        if _scan_mode == -1:
            ...
            if no_einsum:
                ...
                self.x_proj_weight = self.x_proj_weight.repeat(K, 1, 1)            # 20251005
                ...
                dts, Bs, Cs = torch.split(x_dbl, [R, N, N], dim=2)
                if hasattr(self, "dt_projs_weight"):
                    dts = torch.einsum("b k r l, k d r -> b k d l", dts, self.dt_projs_weight.to(dts.device))

            xs = xs.view(B, -1, L)
            dts = dts.contiguous().view(B, -1, L)
            As = -self.A_logs.to(xs.device).to(torch.float).exp()  # (k * c, d_state)
            Ds = self.Ds.to(xs.device).to(torch.float)  # (K * c)
            Bs = Bs.contiguous().view(B, K, N, L)
            Cs = Cs.contiguous().view(B, K, N, L)
            delta_bias = self.dt_projs_bias.to(xs.device).view(-1).to(torch.float)
    ...
    def forwardv4(self, x: torch.Tensor, **kwargs):
        x = self.in_proj(x)
        if not self.disable_z:
            x, z = x.chunk(2, dim=(1 if self.channel_first else -1))  # (b, h, w, d)
            if not self.disable_z_act:
...
class SS2D(nn.Module, SS2Dv0, SS2Dv2, SS2Dv3, SS2Dv4, SS2Dm0):
    def __init__(
        self,
        # basic dims ===========
        d_model=96,
        ...
        **kwargs,
        ...
        elif "v4" in forward_type:            # 20251005
            self.__initv4__(**kwargs)
        else:
